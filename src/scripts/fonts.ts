...
const FILE_SUFFIX = 'fonts.css';

class Fonts {
  container: HTMLElement;

  constructor() {
    this.container = document.querySelector('.fonts-container');
  }
  ...
  update() {
    ...
    const groups = this.getVariables();
  ...
  getVariables(): CSSVarGroup[] {
    const groups: CSSVarGroup[] = [];
    for (const s of document.styleSheets) {
      try {
        if (s.href.lastIndexOf(FILE_SUFFIX) !== s.href.length - FILE_SUFFIX.length) {
          continue;
        }

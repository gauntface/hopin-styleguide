const CLASSNAMES_CONTAINER = '__hopin__js-classnames';
...
class ClassName {
                ...
                for (const r of cssStylesheet.cssRules) {
                    ...
                    if (r instanceof CSSStyleRule) {
                        ...
                        for (const s of selectors) {
                            ...
                            switch (firstChar) {
                                case '.':
                                    const regex = /^\.(?:__([a-z0-9-]*)__)?([cul])-((?:[a-z0-9]|-(?=[a-z0-9]))+)(?:__((?:[a-z0-9]|-(?=[a-z0-9]))+))?(?:--((?:[a-z0-9]|-(?=[a-z0-9]))+))?(?::(?:hover|active|focus|visited))*?$/;
    ...
    render() {
        const containerElement = document.querySelector(`.${CLASSNAMES_CONTAINER}`);
        if (!containerElement) {
            console.warn(`Unable to find container with class ${CLASSNAMES_CONTAINER}`)
            return;
        }
